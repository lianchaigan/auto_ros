    ...
    )
{
    ...
    memset( pUSBHanlde, 0, sizeof(USBD_HANDLE) );
//pUSBHanlde->id = 5;
//    pUSBHanlde->dev_config = 0x12345678;
    /* Assign USBD Descriptors */
    if( 0 != pdesc )
    {
        pUSBHanlde->pDesc = pdesc;
    }

    /* Set Device initial State */
    pUSBHanlde->dev_state  = USBD_STATE_DEFAULT;
    pUSBHanlde->id = id;

    /* Initialize low level driver */
    USBD_LL_Init( pUSBHanlde, pHpcd_USB_FS );
...
USBD_STS USBD_SetClassConfig( USBD_HANDLE  *pdev, uint8_t cfgidx )
{
    ...
    if(pdev->pClass != NULL)
    {
        ...
        if( 0 == pdev->pClass->Init( pdev, cfgidx ) )
        {
            ret = USBD_OK;
        }
    }
...
USBD_STS USBD_LL_Reset( USBD_HANDLE  *pdev )
{
    ...
    pdev->pEpOut[0].maxpacket = USB_MAX_EP0_SIZE;
    ...
    pdev->pEpIn[0].maxpacket = USB_MAX_EP0_SIZE;

    /* Upon Reset call user call back */
    pdev->dev_state = USBD_STATE_DEFAULT;

    if( 0 != pdev->pClassData )
    {
    pdev->pClass->DeInit(pdev, pdev->dev_config); 
    }
