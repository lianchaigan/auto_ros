...
typedef void USB_FS_CB_DATA_RDY( void );
...
typedef struct _USBD_CUSTOM_HID_Itf
{
  ...
  int8_t (* Init)          (void);
  int8_t (* DeInit)        (void);
  int8_t (* OutEvent)      (uint8_t, uint8_t, uint8_t * );   
...
/** @defgroup USB_CORE_Exported_Functions
  * @{
  */ 
uint8_t USBD_CUSTOM_HID_SendReport( USBD_HANDLE *pdev, uint8_t *report, uint16_t len );
uint8_t  USBD_CUSTOM_HID_RegisterInterface( USBD_HANDLE *pdev, USBD_CUSTOM_HID_ItfTypeDef *fops );
