...
typedef struct _tagData_header_t
{
    long size;
    uint32_t checksum;
    unsigned int key;
}MPU_DATA_HEADER, *PMPU_DATA_HEADER;


typedef  struct _tagData_storage_t
{
    ...
    MPU_DATA_HEADER hd[NUM_STORAGE_BOXES]; /**< Header info for each entity */
}MPU_DATA_STORAGE, *PMPU_DATA_STORAGE;


static MPU_DATA_STORAGE ds;
...
void inv_init_storage_manager()
{
    ...
    ds.total_size = sizeof(MPU_DATA_HEADER);
}
	...
	)
{
    ...
    ds.total_size += size + sizeof(MPU_DATA_HEADER);
    ds.num++;
...
inv_error_t inv_load_mpl_states( const unsigned char *data, size_t length )
{
    ...
    uint32_t checksum;
    long len;
	long HeaderSize = sizeof(MPU_DATA_HEADER);
...
inv_error_t inv_save_mpl_states( unsigned char *data, size_t sz )
{
    ...
    int kk;
    MPU_DATA_HEADER *hd;
    int HeaderSize = sizeof(MPU_DATA_HEADER);
    ...
    hd = (MPU_DATA_HEADER *)data;
    hd->checksum = inv_checksum( data + HeaderSize, ds.total_size - HeaderSize );
    hd->key = DEFAULT_KEY;
