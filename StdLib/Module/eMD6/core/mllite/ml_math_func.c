...
static long long  g_TempA = 0;
static long long  g_TempB = 0;
...
{
    rot[0] = inv_q29_mult(quat[1], quat[1]) + inv_q29_mult(quat[0], quat[0]) - 1073741824L;
    ...
    rot[4] = inv_q29_mult(quat[2], quat[2]) + inv_q29_mult(quat[0], quat[0]) - 1073741824L;
    ...
    rot[8] = inv_q29_mult(quat[3], quat[3]) + inv_q29_mult(quat[0], quat[0]) - 1073741824L;
}
...
float inv_matrix_det(float *p, int *n)
{
    ...
    for (i = 0, j = 0; j < m; j++)
	{
        *n = m;
        ...
        sum = sum + *(p + 6 * i + j) * SIGNM(i + j) * inv_matrix_det(&d[0][0], n);
    }
...
double inv_matrix_detd(double *p, int *n)
{
    ...
    for (i = 0, j = 0; j < m; j++)
	{
        *n = m;
        ...
        sum = sum + *(p + 6 * i + j) * SIGNM(i + j) * inv_matrix_detd(&d[0][0], n);
    }
