...
typedef void (*DWT_CB_FUNC)( const DWT_CB_DATA *pData );


#pragma pack(push, 1)
 /* Structure typedef: dwt_config_t
 Structure for setting device configuration via DWT_Config() function */
...
typedef struct
{
    ...
    DWT_CB_DATA cbData;          // Callback data structure
    DWT_CB_FUNC    cbTxDone;          // Callback for TX confirmation event
    DWT_CB_FUNC    cbRxOk;            // Callback for RX good frame event
    DWT_CB_FUNC    cbRxTo;            // Callback for RX timeout events
    DWT_CB_FUNC    cbRxErr;           // Callback for RX error events
} DWT_HANDLE, *PDWT_HANDLE;
...
/******************************************************************************
 @Description 	: This is used to return the read part ID of the device.
				  DWT_Init() must be called prior to this function so that
				  it can return a relevant value.

 @param			: 

 @Return		: 32 bit part ID value as programmed in the factory
...
/******************************************************************************
 @Description 	: This is used to return the read device type and revision 
				  information of the DW1000 device (MP part is 0xDECA0130)

 @param			: 

 @Return		: the read value which for DW1000 is 0xDECA0130
...
/******************************************************************************
 @Description 	: This function enables/disables the fine grain TX sequencing
				  (enabled by default).
...
/******************************************************************************
 @Description 	: This is used to enable GPIO for external LNA or PA 
				  functionality - HW dependent, consult the DW1000 User Manual.
				  This can also be used for debug as enabling TX and RX GPIOs
				  is quite handy to monitor DW1000's activity.
				  Enabling PA functionality requires that fine grain TX 
				  sequencing is deactivated. This can be done using
				  DWT_SetFineGrainTxSeq().

 @param			: lna - 1 to enable LNA functionality, 0 to disable it
				  pa - 1 to enable PA functionality, 0 to disable it
 @Return		: None
...
/******************************************************************************
 @Description 	: This is used to set GPIO direction as an input (1)
				  or output (0)

 @param			: gpioNum   - this is the GPIO to configure
							- see GxM0... GxM8 in the deca_regs.h file

				  direction - this sets the GPIO direction 
				            - see GxP0... GxP8 in the deca_regs.h file
 @Return		: None
...
/******************************************************************************
 @Description 	: This is used to set GPIO value as (1) or (0) only applies 
				  if the GPIO is configured as output
...
/******************************************************************************
 @Description 	: This function initiates communications with the DW1000 
				  transceiver and reads its DEV_ID register (address 0x00) to
				  verify the IC is one supported by this software
				  (e.g. DW1000 32-bit device ID value is 0xDECA0130). Then it
				  does any initial once only device configurations needed for 
				  use and initialize as necessary any static data items
 ...
 @param			: config -  specifies what configuration to load
							...
							DWT_LOADNONE      0x0 - do not load any values 
											        from OTP memory
 ...
 * no return value
 */
void DWT_EnableFrameFilter( DWT_HANDLE *pHandle, uint16 bitmask );
