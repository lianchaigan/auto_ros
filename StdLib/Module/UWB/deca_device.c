...
/* Defines for ACK request bitmask in DATA and MAC COMMAND frame 
control (first byte) - Used to detect AAT bit wrongly set. */
#define FCTRL_ACK_REQ_MASK 0x20

/* Frame control maximum length in bytes */
#define FCTRL_LEN_MAX 2

/* define so API checks config input parameters */
...
/*****************************************************************************
 Local Functions
******************************************************************************/
static void deca_Delayms( int ms );
#define deca_sleep( x ) deca_Delayms( ( x ) )

static void dwt_EnableClocks( int clocks );
static void dwt_Configlde( int prf );
static void dwt_LoadUcodeFromROM( void );
static uint32 dwt_OtpRead( uint32 address );
static uint32 dwt_OtpProgWord32( uint32 data, uint16 address );
static void dwt_AonArrayUpload( void );
static void dwt_AonConfigUpload( void );

/*****************************************************************************
 Implementation
******************************************************************************/

/* This function initiates communications with the DW1000 transceiver
...
NOTES:
1.this function needs to be run before DWT_Configsleep, also the SPI frequency has to be < 3MHz
...
int DWT_Init( DWT_HANDLE *pHandle, uint16 config )
{
    uint16 otp_addr = 0;
    uint32 ldo_tune = 0;
    uint16 rega = 0;
    ...
    pHandle->dblbuffon = 0; // Double buffer mode off by default
    pHandle->wait4resp = 0;
    pHandle->sleep_mode = 0;
    ...
    DWT_SoftReset( pHandle );

    /* NOTE: set system clock to XTI - this is necessary to make sure 
	the values read by dwt_OtpRead are reliable */
    dwt_EnableClocks( FORCE_SYS_XTI );

    /* Configure the CPLL lock detect */
    dwt_write8bitoffsetreg( EXT_SYNC_ID, EC_CTRL_OFFSET, EC_CTRL_PLLLCK );

    /* Read OTP revision number */
    otp_addr = dwt_OtpRead( XTRIM_ADDRESS ) & 0xffff; /* Read 32 bit value, XTAL trim val is in low octet-0 (5 bits) */
    pHandle->otprev = ( otp_addr >> 8 ) & 0xff;       /* OTP revision is next byte */

    /* Load LDO tune from OTP and kick it if there is a value actually programmed. */
    ldo_tune = dwt_OtpRead( LDOTUNE_ADDRESS );
    if ( 0 != ( ldo_tune & 0xFF ) )
    {
        /* Kick LDO tune */

        /* Set load LDE kick bit */
        dwt_write8bitoffsetreg( OTP_IF_ID, OTP_SF, OTP_SF_LDO_KICK );

        /* LDO tune must be kicked at wake-up */
        pHandle->sleep_mode |= AON_WCFG_ONW_LLDO;
    }

    /* Load Part and Lot ID from OTP */
    pHandle->partID = dwt_OtpRead( PARTID_ADDRESS );
    pHandle->lotID = dwt_OtpRead( LOTID_ADDRESS );

    /* XTAL trim value is set in OTP for DW1000 module and EVK/TREK boards 
	but that might not be the case in a custom design */
    pHandle->init_xtrim = otp_addr & 0x1F;

    /* A value of 0 means that the crystal has not been trimmed */
    if ( 0 == pHandle->init_xtrim )
    {
        /* Set to mid-range if no calibration value inside */
        pHandle->init_xtrim = FS_XTALT_MIDRANGE;
    }

    /* Configure XTAL trim */
    dwt_setxtaltrim( pHandle->init_xtrim );

    /* Load leading edge detect code */
    if ( 0 != ( config & DWT_LOADUCODE ) )
    {
        dwt_LoadUcodeFromROM();

        /* microcode must be loaded at wake-up */
        pHandle->sleep_mode |= AON_WCFG_ONW_LLDE;
    }
    else
    {
        /* Should disable the LDERUN enable bit in 0x36, 0x4 */

        rega = dwt_read16bitoffsetreg( PMSC_ID, ( PMSC_CTRL1_OFFSET + 1 ) );
    ...
    dwt_EnableClocks( ENABLE_ALL_SEQ ); /* Enable clocks for sequencing */
    ...
    pHandle->sysCFGreg = DWT_Read32BitReg( SYS_CFG_ID ); /* Read sysconfig register */

    return DWT_SUCCESS;
}

uint8 DWT_OtpRevision( DWT_HANDLE *pHandle )
{
    /* This is used to return the read OTP revision */
    ASSERT( 0 != pHandle );

    return pHandle->otprev;
}

void DWT_SetFineGrainTxSeq( int enable )
{
    /* This function enables/disables the fine grain TX sequencing (enabled by default). */

    /*  1 to enable fine grain TX sequencing, 0 to disable it */
    if ( 0 != enable )
    {
 ...
 *
 * NOTE: DWT_Init() must be called prior to this function so that it can return a relevant value.
 *
 * input parameters
 *
 * output parameters
 ...
 */
uint32 DWT_GetpPartID( DWT_HANDLE *pHandle )
{
    ASSERT( 0 != pHandle );

    return pHandle->partID;
}
 ...
 */
uint32 DWT_GetLotID( DWT_HANDLE *pHandle )
{
    ASSERT( 0 != pHandle );

    return pHandle->lotID;
}
 ...
 * no return value
 */
void DWT_Configtxrf( dwt_txconfig_t *config )
{
 ...
 * no return value
 */
void DWT_Config( DWT_HANDLE *pHandle, dwt_config_t *config )
{
    ...
    {
        pHandle->sysCFGreg |= SYS_CFG_RXM110K;
    ...
    pHandle->longFrames = config->phrMode;

    pHandle->sysCFGreg &= ~SYS_CFG_PHR_MODE_11;
    pHandle->sysCFGreg |= ( SYS_CFG_PHR_MODE_11 & ( config->phrMode << SYS_CFG_PHR_MODE_SHFT ) );
    pHandle->sysCFGreg |= SYS_CFG_RXAUTR; /* Enable auto received mode on */
    DWT_Write32BitReg( SYS_CFG_ID, pHandle->sysCFGreg );

    // Set the lde_replicaCoeff
    dwt_write16bitoffsetreg( LDE_IF_ID, LDE_REPC_OFFSET, reg16 );

    dwt_Configlde( prfIndex );
    ...
    pHandle->txFCTRL = ( ( config->txPreambLength | config->prf ) << TX_FCTRL_TXPRF_SHFT ) | ( config->dataRate << TX_FCTRL_TXBR_SHFT );
    DWT_Write32BitReg( TX_FCTRL_ID, pHandle->txFCTRL );
 ...
 * no return value
 */
void DWT_WriteTxFrameCtrl( DWT_HANDLE *pHandle, uint16 txFrameLength, uint16 txBufferOffset, int ranging )
{
    ...
    // pHandle->txFCTRL has kept configured bit rate information
    uint32 reg32 = pHandle->txFCTRL | txFrameLength | ( txBufferOffset << TX_FCTRL_TXBOFFS_SHFT ) | ( ranging << TX_FCTRL_TR_SHFT );
    DWT_Write32BitReg( TX_FCTRL_ID, reg32 );
} // end DWT_WriteTxFrameCtrl()
...
void dwt_readaccdata( uint8 *buffer, uint16 len, uint16 accOffset )
{
    ...
    dwt_EnableClocks( READ_ACC_ON );

    dwt_readfromdevice( ACC_MEM_ID, accOffset, len, buffer );

    dwt_EnableClocks( READ_ACC_OFF ); /* Revert clocks back */
}
 ...
 * no return value
 */
void DWT_EnableFrameFilter( DWT_HANDLE *pHandle, uint16 enable )
{
...
void DWT_SetPanID( uint16 panID )
{
    ...
    dwt_readfromdevice( PANADR_ID, PANADR_PAN_ID_OFFSET, 2U, buf );
}
 ...
 * no return value
 */
void DWT_SetAddr16( uint16 shortAddress )
{
...
void dwt_otpread( uint32 address, uint32 *array, uint8 length )
{
    ...
    dwt_EnableClocks( FORCE_SYS_XTI ); // NOTE: Set system clock to XTAL - this is necessary to make sure the values read by dwt_OtpRead are reliable

    for ( i = 0; i < length; i++ )
    {
        array[i] = dwt_OtpRead( address + i );
    }

    dwt_EnableClocks( ENABLE_ALL_SEQ ); // Restore system clock to PLL
...
static uint32 dwt_OtpProgWord32( uint32 data, uint16 address )
{
    ...
    /* function to program the OTP memory. Ensure that MR,MRa,MRb are reset to 0.
...
int dwt_otpwriteandverify( uint32 value, uint16 address )
{
    ...
    dwt_EnableClocks( FORCE_SYS_XTI ); //set system clock to XTI
    ...
    while ( 1 )
    {
        dwt_OtpProgWord32( value, address );

        if ( dwt_OtpRead( address ) == value )
        {
            break;
        }
        retry++;
    ...
    if ( dwt_OtpRead( address ) != value ) // If this does not pass please check voltage supply on VDDIO
    {
        prog_ok = DWT_ERROR;
    }
...
static void dwt_AonConfigUpload( void )
{
    /* This function uploads always on (AON) configuration,
	 as set in the AON_CFG0_OFFSET register. */
...
static void dwt_AonArrayUpload( void )
{
    /* This function uploads always on (AON) data array and configuration.
	Thus if this function is used, then dwt_AonConfigUpload
	is not necessary. The DW1000 will go so SLEEP straight after this
	if the DWT_SLP_EN has been set. */
 ...
 * @fn dwt_entersleep()
 *
 * @brief This function puts the device into deep sleep or sleep. DWT_Configsleep() should be called first
...
/*! ------------------------------------------------------------------------------------------------------------------
 * @fn DWT_Configsleepcnt()
 *
 * @brief sets the sleep counter to new value, this function programs the high 16-bits of the 28-bit counter
 *
 * NOTE: this function needs to be run before DWT_Configsleep, also the SPI frequency has to be < 3MHz
 *
 * input parameters
 ...
 * no return value
 */
void DWT_Configsleepcnt( uint16 sleepcnt )
{
    ...
    dwt_EnableClocks( ENABLE_ALL_SEQ );
}
 ...
 *
 * NOTE: this function needs to be run before DWT_Configsleepcnt, so that we know what the counter units are
 *
 * input parameters
 *
 * output parameters
 ...
 * no return value
 */
void DWT_Configsleep( DWT_HANDLE *pHandle, uint16 mode, uint8 wake )
{
    ASSERT( 0 != pHandle );

    // Add predefined sleep settings before writing the mode
    mode |= pHandle->sleep_mode;
 ...
 * transmission the device will enter deep sleep mode. The DWT_Configsleep() function
...
{
    /* wake up the device from sleep mode using the SPI read,
...
static void dwt_Configlde( int prfIndex )
{
    ...
    /* this is the PRF index (0 or 1) 0 corresponds to 16 and 1 to 64 PRF */
    if ( prfIndex )
    {
 ...
 * no return value
 */
void DWT_SetSmartTxPower( DWT_HANDLE *pHandle, int enable )
{
    ASSERT( 0 != pHandle );
    ...
    if ( enable )
    {
        pHandle->sysCFGreg &= ~( SYS_CFG_DIS_STXP );
    }
    else
    {
        pHandle->sysCFGreg |= SYS_CFG_DIS_STXP;
 ...
 * no return value
 */
void dwt_enableautoack( DWT_HANDLE *pHandle, uint8 responseDelayTime )
{
    ASSERT( 0 != pHandle );
    ...
    // Enable auto ACK
    pHandle->sysCFGreg |= SYS_CFG_AUTOACK;
 ...
 * no return value
 */
void dwt_setdblrxbuffmode( DWT_HANDLE *pHandle, int enable )
{
    ASSERT( 0 != pHandle );
    ...
    if ( enable )
    {
        ...
        pHandle->sysCFGreg &= ~SYS_CFG_DIS_DRXB;
        pHandle->dblbuffon = 1;
    }
    else
    {
 ...
 * no return value
 */
void dwt_setcallbacks( DWT_HANDLE *pHandle, DWT_CB_FUNC cbTxDone, DWT_CB_FUNC cbRxOk, DWT_CB_FUNC cbRxTo, DWT_CB_FUNC cbRxErr )
{
    ASSERT( 0 != pHandle );

    pHandle->cbTxDone = cbTxDone;
    pHandle->cbRxOk = cbRxOk;
    pHandle->cbRxTo = cbRxTo;
    pHandle->cbRxErr = cbRxErr;
}
 ...
 * no return value
 */
void dwt_isr( DWT_HANDLE *pHandle )
{
    uint32 status = pHandle->cbData.status = DWT_Read32BitReg( SYS_STATUS_ID ); // Read status register low 32bits
    ...
    if ( status & SYS_STATUS_RXFCG )
    {
        ...
        if ( pHandle->longFrames == 0 )
        {
            len &= RX_FINFO_RXFLEN_MASK;
        }
        pHandle->cbData.datalength = len;

        // Report ranging bit
        if ( finfo16 & RX_FINFO_RNG )
        {
            pHandle->cbData.rx_flags |= DWT_CB_DATA_RX_FLAG_RNG;
        }

        // Report frame control - First bytes of the received frame.
        dwt_readfromdevice( RX_BUFFER_ID, 0, FCTRL_LEN_MAX, pHandle->cbData.fctrl );
        ...
        // This issue is not documented at the time of writing this code. It should be in next release of DW1000 User Manual (v2.09, from July 2016).
        if ( ( status & SYS_STATUS_AAT ) && ( ( pHandle->cbData.fctrl[0] & FCTRL_ACK_REQ_MASK ) == 0 ) )
        {
            DWT_Write32BitReg( SYS_STATUS_ID, SYS_STATUS_AAT ); // Clear AAT status bit in register
            pHandle->cbData.status &= ~SYS_STATUS_AAT;          // Clear AAT status bit in callback data register copy
            pHandle->wait4resp = 0;
        }

        // Call the corresponding callback if present
        if ( pHandle->cbRxOk != NULL )
        {
            pHandle->cbRxOk( &pHandle->cbData );
        }

        if ( pHandle->dblbuffon )
        {
    ...
    if ( status & SYS_STATUS_TXFRS )
    {
        ...
        if ( ( status & SYS_STATUS_AAT ) && pHandle->wait4resp )
        {
            DWT_ForceTrxOff( pHandle ); // Turn the RX off
            DWT_ResetRx();              // Reset in case we were late and a frame was already being received
        }

        // Call the corresponding callback if present
        if ( pHandle->cbTxDone != NULL )
        {
            pHandle->cbTxDone( &pHandle->cbData );
        }
    }
    ...
    if ( status & SYS_STATUS_ALL_RX_TO )
    {
        ...
        DWT_ForceTrxOff( pHandle );
        DWT_ResetRx();

        // Call the corresponding callback if present
        if ( pHandle->cbRxTo != NULL )
        {
            pHandle->cbRxTo( &pHandle->cbData );
        }
    }
    ...
    if ( status & SYS_STATUS_ALL_RX_ERR )
    {
        ...
        // Call the corresponding callback if present
        if ( pHandle->cbRxErr != NULL )
        {
            pHandle->cbRxErr( &pHandle->cbData );
        }
    }
}
 ...
 * no return value
 */
void dwt_lowpowerlistenisr( DWT_HANDLE *pHandle )
{
    uint32 status = pHandle->cbData.status = DWT_Read32BitReg( SYS_STATUS_ID ); // Read status register low 32bits
    uint16 finfo16;
    uint16 len;
 ...
 */
int DWT_StartTx( DWT_HANDLE *pHandle, uint8 mode )
{
 ...
 * no return value
 */
void DWT_ForceTrxOff( DWT_HANDLE *pHandle )
{
 ...
 *           - DWT_Configsleep() to configure long sleep phase. "mode" parameter should at least have DWT_PRESRV_SLEEP,
 *             DWT_CONFIG and DWT_RX_EN set and "wake" parameter should at least have both DWT_WAKE_SLPCNT and DWT_SLP_EN set.
 *           - dwt_calibratesleepcnt() and DWT_Configsleepcnt() to define the "long sleep" phase duration.
 ...
 *           - DWT_EnableInterrupt() to activate RX good frame interrupt (DWT_INT_RFCG) only.
 *       When configured, low-power listening mode can be triggered either by putting the DW1000 to sleep (using
 *       dwt_entersleep()) or by activating reception (using DWT_EnableRx()).
 *
 ...
 */
int DWT_EnableRx( DWT_HANDLE *pHandle, int mode )
{
    uint16 temp;
    uint8 temp1;
    ...
    if ( mode & DWT_START_RX_DELAYED ) // check for errors
    {
        ...
        {
            DWT_ForceTrxOff( pHandle ); // turn the delayed receive off
 ...
 * no return value
 */
void dwt_setrxtimeout( DWT_HANDLE *pHandle, uint16 time )
{
    uint8 temp;
    ...
    if ( time > 0 )
    {
        ...
        pHandle->sysCFGreg |= SYS_CFG_RXWTOE;
 ...
 * no return value
 */
void DWT_EnableInterrupt( uint32 bitmask, uint8 enable )
{
    decaIrqStatus_t stat;
    uint32 mask;
 ...
 * no return value
 */
void DWT_SoftReset( DWT_HANDLE *pHandle )
{
    ASSERT( 0 != pHandle );
 ...
 */
uint8 dwt_getinitxtaltrim( DWT_HANDLE *pHandle )
{
    ASSERT( 0 != pHandle );

    return pHandle->init_xtrim;
}
...
void dwt_configcontinuousframemode( uint32 framerepetitionrate )
{
    ...
    // Configure TX clocks
    //
    dwt_EnableClocks( FORCE_SYS_PLL );
    dwt_EnableClocks( FORCE_TX_PLL );
...
uint16 dwt_readtempvbat( uint8 fastSPI )
{
    ...
    {
        dwt_EnableClocks( FORCE_SYS_XTI ); // NOTE: set system clock to XTI - this is necessary to make sure the values read are reliable
        // Read voltage and temperature.
        dwt_readfromdevice( TX_CAL_ID, TC_SARL_SAR_LVBAT_OFFSET, 2, wr_buf );
        // Default clocks (ENABLE_ALL_SEQ)
        dwt_EnableClocks( ENABLE_ALL_SEQ ); // Enable clocks for sequencing
 ...
 * wakeup if DWT_TANDV bit is set in mode parameter of DWT_Configsleep
 *
 * input parameters:
 *
 * output parameters:
